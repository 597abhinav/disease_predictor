symptoms_list = ["itching","skin_rash","nodal_skin_eruptions","continuous_sneezing","shivering","chills","joint_pain",
	...
	"patches_in_throat","irregular_sugar_level","cough","high_fever","sunken_eyes","breathlessness","sweating","dehydration",
...
def get_symptoms(request, user_symptoms):
	for i in request.form.getlist('symptoms'):
		user_symptoms[int(i)] = 1
	for i in request.form.getlist('symptoms1'):
		user_symptoms[int(i)] = 1
	for i in request.form.getlist('symptoms2'):
		user_symptoms[int(i)] = 1
